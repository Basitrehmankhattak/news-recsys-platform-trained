...
def apply_netflix_ui():
    st.markdown(
        ...
        /* Remove extra padding from empty Streamlit blocks */
...
# 6. Main Logic - Use Streamlit container instead of raw HTML
with st.container():
    ...
    with col2:
        if st.session_state.anonymous_id:
            ...
            st.markdown("### Sign In")
            handle = st.text_input("Username", placeholder="e.g. basit1", key="login_handle")
            st.markdown('<div style="text-align:center; margin:5px" class="muted">OR</div>', unsafe_allow_html=True)
            anonymous = st.text_input("Account ID", placeholder="Full ID here", key="login_anon")
            c1, c2 = st.columns(2)
            with c1:
            ...
            new_handle = st.text_input("Choose username", placeholder="basit1", key="create_handle")
            c1, c2 = st.columns(2)
            with c1:
                if st.button("Back", use_container_width=True):
                    st.session_state.account_view = "choice"
